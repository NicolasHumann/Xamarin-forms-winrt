...
namespace Xamarin.Forms.Platform.WinRT
{
    class ThreadTimer : ITimer, IDisposable
    {
        ...
        {
            _timer = new Timer(s => callback(s), null, 0, 0);
        }
        ...
        {
            _timer = new Timer(s => callback(s), state, TimeSpan.FromMilliseconds(dueTime), TimeSpan.FromMilliseconds(period));
        }
        ...
        {
            _timer.Change(TimeSpan.FromMilliseconds(dueTime), TimeSpan.FromMilliseconds(period));
        }
